...
function checkValueAndType(mySet, value) {
  return mySet.has(value) ? `Множина має значення "${value}" типу "${typeof value}"` : ` Множина не має значення "${value}"` ;
...
function setToArray(mySet) {
  ...
  const stringElements = arrayFromSet.filter(item => typeof item === 'string');
...
console.log(areDisjoint(new Set([1, 2, 7]), new Set([3, 4, 5])));
// Виведе: false
...
function iterateSet(set) {
  ...
  for (let entrie of entriesIterator) {
    console.log(entrie);
  }
